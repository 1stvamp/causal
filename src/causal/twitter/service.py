...
def get_items(user, since, model_instance=None):
    ...
    since_id = None
    try:
        timeline = api.search("from:%s" % (api.me().screen_name),
        ...
        if len(timeline):
            ...
            try:
                # add on the rest of the tweets
                timeline = timeline + api.user_timeline(count=200, since_id=since_id, include_rts='true', page=1)
                
                # check if hit the 200 limit, if so go get another 200
                if len(timeline) == 200:
                    more_tweets = api.user_timeline(count=200, since_id=since_id, include_rts='true', page=2)
