...
def verify_auth(request):
    ...
    request_token.save()
    return_url = request.session.get('helios_foursquare_oauth_return_url', None) or 'history'
    generate_access_token(service, request_token)
    return redirect(return_url)

@login_required(redirect_field_name='redirect_to')
def auth(request):
    request.session['helios_foursquare_oauth_return_url'] = request.GET.get('HTTP_REFERER', None)
